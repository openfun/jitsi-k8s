...
resource "scaleway_k8s_pool" "default" {
   ...
   min_size            = lookup(var.k8s_default_nodepool_min_nodes, terraform.workspace, 3)
   name                = "default"
   node_type           = lookup(var.k8s_default_nodepool_flavor, terraform.workspace, "GP1-XS")
   size                = lookup(var.k8s_default_nodepool_size, terraform.workspace, 3)
...
resource "scaleway_k8s_pool" "jvb" {
   ...
   # We wait for default pool to be ready before creating the jvb pool,
   # otherwise some kube-system pods created by scaleway might be scheduled
   # on the jibri pool at cluster initialization
   depends_on = [ scaleway_k8s_pool.default ]
}
